    ...
    <q-card v-show="state" class="q-ma-md">
      <q-expansion-item expand-separator>
        <template v-slot:header>
          ...
          <q-item-section>
            <q-item-label v-if="allLogs.length > 1" class="ellipsis">{{ allLogs[allLogs.length -
              2].message}}</q-item-label>
            <q-item-label v-if="allLogs.length > 0" class="ellipsis">{{ allLogs[allLogs.length -
              1].message}}</q-item-label>
          </q-item-section>
        </template>
            ...
            <template v-slot="{ item, index }">
              <q-expansion-item expand-separator :key="index">
                <template v-slot:header>
                  ...
                  <q-item-section>
                    <q-item-label v-if="item.logs.length > 0" class="ellipsis">{{ item.logs[item.logs.length -
                      1].message}}</q-item-label>
...
export default {
  ...
  methods: {
    ...
    performModify() {
      ...
      this.results = []
      this.state = 'running'
      this.$socket.emit('PERFORM_MODIFY')
    },
